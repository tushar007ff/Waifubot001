...
async def reply_character_photo(update: Update, context: CallbackContext) -> None:
    chat_id = update.effective_chat.id

    # Get character information from your data source
    character = await get_character_info()  # Implement this function to fetch character info

    # Send photo and character information
    await context.bot.send_photo(
        chat_id=chat_id,
        photo=character['img_url'],
        caption=f"𝗡𝗮𝗺𝗲: {character['name']}\n𝗔𝗻𝗶𝗺𝗲: {character['anime']}\n𝗜𝗗: {character['id']}",
        parse_mode='Markdown'
    )
...
def main() -> None:
    ...
    application.add_handler(CommandHandler('info', reply_character_photo, block=False))
